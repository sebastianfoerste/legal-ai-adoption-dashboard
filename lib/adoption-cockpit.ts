...
export type AdoptionCockpitDeckSlide = {
  title: string;
  takeaway: string;
  evidence: string[];
  reviewGate: string;
...
export type AdoptionCockpitPresentationDeck = {
  schema: "legal-ai-adoption.adoption-cockpit-deck.v1";
  sourceMode: "local_synthetic_json";
  generatedFrom: string[];
  title: string;
  audience: string;
  exportFormats: Array<"pptx" | "pdf" | "markdown_report">;
  slides: AdoptionCockpitDeckSlide[];
  evidenceList: string[];
...
export type AdoptionCockpitReviewLayer = {
  key:
...
export type AdoptionCockpitReviewSkill = {
  id: string;
  label: string;
  objective: string;
  outputSchema: string[];
...
export type AdoptionCockpitProductSurfaceCoverage = {
  surface:
    | "Agent"
    | "Review Rows"
    | "Workflows"
...
export type AdoptionCockpitAdoptionControls = {
  schema: "legal-ai-adoption.adoption-cockpit-controls.v1";
  sourceMode: "local_synthetic_json";
  generatedFrom: string[];
  reviewLayers: AdoptionCockpitReviewLayer[];
  agentPlan: Record<"plan" | "execute" | "review" | "deliver", string>;
  skills: AdoptionCockpitReviewSkill[];
  ...
  productSurfaceCoverage: AdoptionCockpitProductSurfaceCoverage[];
  adoptionSignals: {
...
export type AdoptionCockpitReport = {
  schema: "legal-ai-adoption.adoption-cockpit.v1";
  generatedAt: string;
  ...
  presentationDeck: AdoptionCockpitPresentationDeck;
  governancePanel: GovernancePanel;
  reviewGateControl: ReviewGate;
  adoptionControls: AdoptionCockpitAdoptionControls;
};
...
export function adoptionCockpitReport(): AdoptionCockpitReport {
  ...
  const gate = reviewGateControl(
    "Synthetic Adoption Cockpit report remains draft-only until account owner review.",
  );
  const adoptionControls = buildAdoptionControls({
    activeUsers,
    workflowRuns,
    reviewTables,
    draftOutputs,
...
function buildPresentationDeck({
  ...
  releaseActions: ReleaseAction[];
  promptReadiness: PromptReadinessItem[];
}): AdoptionCockpitPresentationDeck {
  const reviewGate =
  ...
  return {
    ...
    reviewGateControl: slideGate,
    markdownReport: renderAdoptionCockpitMarkdown({
      leadershipBrief,
      verificationRate,
      productUsage,
      benchmarks,
...
export function renderAdoptionCockpitMarkdown({
  leadershipBrief,
  verificationRate,
  productUsage,
  benchmarks,
...
function buildAdoptionControls({
  activeUsers,
  workflowRuns,
  reviewTables,
  draftOutputs,
  ...
  presentationDeck: AdoptionCockpitPresentationDeck;
  governancePanel: GovernancePanel;
}): AdoptionCockpitAdoptionControls {
  ...
  const reviewGate =
    "Synthetic adoption signal only. Account owner review is required before enablement or client-facing use.";
  const productSurfaceCoverage: AdoptionCockpitProductSurfaceCoverage[] = [
    {
      surface: "Agent",
  ...
  return {
    ...
    externalActionAllowed: false,
    reviewNotice:
      "Local synthetic Adoption Cockpit evidence only. Account actions remain blocked until owner review.",
  };
}
