...
export default async function AccountHealthPage({
  ...
  const adoptionCockpit = adoptionCockpitReport();
  ...
  return (
    <main className="mx-auto max-w-5xl px-6 py-10">
      ...
      <section className="mb-10 rounded-lg border border-gray-200 bg-white p-5" aria-labelledby="adoption-cockpit-heading">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-violet-600">Adoption Cockpit</p>
            <h2 id="adoption-cockpit-heading" className="mt-1 text-lg font-semibold text-gray-900">
          ...
          <span className="rounded border border-gray-200 bg-gray-50 px-2 py-1 font-mono text-xs text-gray-600">
            {adoptionCockpit.schema}
          </span>
        </div>

        <div className="mt-5 grid grid-cols-2 gap-3 md:grid-cols-6">
          <CommandMetric label="Active users" value={adoptionCockpit.activeUsers} />
          <CommandMetric label="Workflow runs" value={adoptionCockpit.workflowRuns} />
          <CommandMetric label="Review tables" value={adoptionCockpit.reviewTables} />
          <CommandMetric label="Draft outputs" value={adoptionCockpit.draftOutputs} />
          <CommandMetric label="Verified outputs" value={adoptionCockpit.verifiedOutputs} />
          <CommandMetric label="Blocked outputs" value={adoptionCockpit.blockedOutputs} tone="text-red-700" />
        </div>
        ...
        <div className="mt-6 overflow-hidden rounded-lg border border-gray-100">
          <div className="flex flex-wrap items-start justify-between gap-3 border-b border-gray-100 bg-gray-50 px-3 py-2">
            <div>
              <h3 className="text-sm font-semibold text-gray-900">Adoption controls</h3>
              <p className="mt-1 text-xs text-gray-600">{adoptionCockpit.adoptionControls.reviewNotice}</p>
            </div>
            <div className="flex flex-wrap gap-2 text-xs">
              <span className="rounded border border-gray-200 bg-white px-2 py-1 font-mono text-gray-600">
                {adoptionCockpit.adoptionControls.schema}
              </span>
              <span className="rounded border border-red-100 bg-red-50 px-2 py-1 font-semibold text-red-700">
                {adoptionCockpit.adoptionControls.externalActionAllowed ? "external allowed" : "external blocked"}
              </span>
            </div>
          </div>
            ...
            <div className="border-b border-gray-100 p-3 lg:border-b-0 lg:border-r">
              <dl className="grid grid-cols-2 gap-3 text-xs text-gray-600">
                <div>
                  ...
                  <dd>{adoptionCockpit.adoptionControls.sourceMode}</dd>
                </div>
                <div>
                  <dt className="font-semibold text-gray-900">Deck package</dt>
                  <dd>{adoptionCockpit.adoptionControls.adoptionSignals.deckPackageReady ? "ready" : "pending"}</dd>
                </div>
                <div>
                  <dt className="font-semibold text-gray-900">Power groups</dt>
                  <dd>{adoptionCockpit.adoptionControls.adoptionSignals.powerUserGroups}</dd>
                </div>
                <div>
                  <dt className="font-semibold text-gray-900">Review gates</dt>
                  <dd>{adoptionCockpit.adoptionControls.adoptionSignals.reviewGatesTracked}</dd>
                </div>
              </dl>
              <p className="mt-3 text-xs text-gray-600">
                Sources: {adoptionCockpit.adoptionControls.generatedFrom.join(", ")}
              </p>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full min-w-[720px] text-sm">
                ...
                <tbody className="divide-y divide-gray-100">
                  {adoptionCockpit.adoptionControls.productSurfaceCoverage.map((surface) => (
        ...
        <div className="mt-6 grid gap-5 lg:grid-cols-2">
          <div className="overflow-x-auto rounded-lg border border-gray-100">
            ...
            <table className="w-full min-w-[720px] text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {adoptionCockpit.benchmarks.map((benchmark) => (
          ...
          <div className="overflow-x-auto rounded-lg border border-gray-100">
            ...
            <table className="w-full min-w-[720px] text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {adoptionCockpit.releaseActions.map((release) => (
            ...
            <p className="border-t border-gray-100 px-3 py-2 text-xs text-gray-600">
              {adoptionCockpit.releaseActions[0]?.gate}
            </p>
          </div>
        </div>
        ...
        <div className="mt-6 grid gap-5 lg:grid-cols-[1.15fr_0.85fr]">
          <div className="space-y-5">
            <div className="overflow-x-auto rounded-lg border border-gray-100">
              <table className="w-full min-w-[760px] text-sm">
                ...
                <tbody className="divide-y divide-gray-100">
                  {adoptionCockpit.productUsage.map((product) => (
            ...
            <div className="overflow-x-auto rounded-lg border border-gray-100">
              ...
              <table className="w-full min-w-[760px] text-sm">
                ...
                <tbody className="divide-y divide-gray-100">
                  {adoptionCockpit.practiceGroupUsage.map((group) => (
          ...
          <div className="rounded-lg border border-violet-100 bg-violet-50/50 p-4">
            <h3 className="text-sm font-semibold text-gray-900">Ask Adoption Cockpit</h3>
            <p className="mt-2 text-sm leading-6 text-gray-700">
              {adoptionCockpit.leadershipBrief.headline}
            </p>
            <div className="mt-3 rounded-lg border border-violet-100 bg-white p-3">
              <h4 className="text-xs font-bold uppercase tracking-wider text-violet-700">
                {adoptionCockpit.leadershipBrief.title}
              </h4>
              <ul className="mt-2 space-y-1 text-xs text-gray-700">
                {adoptionCockpit.leadershipBrief.slides.map((slide) => (
                  <li key={slide}>{slide}</li>
                ))}
              </ul>
              <p className="mt-2 text-xs text-gray-600">{adoptionCockpit.leadershipBrief.reviewGate}</p>
            </div>
            <div className="mt-3 rounded-lg border border-violet-100 bg-white p-3">
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div>
                  ...
                  <p className="mt-1 text-xs text-gray-600">{adoptionCockpit.presentationDeck.audience}</p>
                </div>
                <span className="rounded border border-gray-200 bg-gray-50 px-2 py-1 font-mono text-[10px] text-gray-600">
                  {adoptionCockpit.presentationDeck.schema}
                </span>
              </div>
              <dl className="mt-3 grid gap-2 text-xs text-gray-700">
                {adoptionCockpit.presentationDeck.slides.map((slide) => (
                  <div key={slide.title}>
              ...
              <p className="mt-3 text-xs text-gray-600">
                Sources: {adoptionCockpit.presentationDeck.generatedFrom.join(", ")}. Formats:{" "}
                {adoptionCockpit.presentationDeck.exportFormats.join(", ")}.
              </p>
              <ul className="mt-2 space-y-1 text-xs text-gray-700">
                {adoptionCockpit.presentationDeck.evidenceList.map((item) => (
                  <li key={item}>{item}</li>
                ))}
              </ul>
              <p className="mt-2 text-xs text-gray-600">{adoptionCockpit.presentationDeck.accountOwnerReviewGate}</p>
              <p className="mt-2 text-xs text-gray-600">{adoptionCockpit.presentationDeck.exportGate}</p>
            </div>
            <div className="mt-3 rounded-lg border border-violet-100 bg-white p-3">
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div>
                  ...
                  <p className="mt-1 text-xs text-gray-600">{adoptionCockpit.governancePanel.schema}</p>
                </div>
                <span className="rounded border border-red-100 bg-red-50 px-2 py-1 text-[10px] font-semibold text-red-700">
                  {adoptionCockpit.governancePanel.reviewGateControl.externalActionAllowed
                    ? "external allowed"
                    : "external blocked"}
                </span>
              </div>
              <dl className="mt-3 grid gap-2 text-xs text-gray-700 sm:grid-cols-2">
                <div>
                  ...
                  <dd>{adoptionCockpit.governancePanel.auditTrailEntries.length}</dd>
                </div>
                <div>
                  <dt className="font-semibold text-gray-900">Source mode</dt>
                  <dd>{adoptionCockpit.governancePanel.sourceMode}</dd>
                </div>
                <div>
                  <dt className="font-semibold text-gray-900">Matter isolation</dt>
                  <dd>{adoptionCockpit.governancePanel.ethicalWalls.matterIsolation}</dd>
                </div>
                <div>
                  <dt className="font-semibold text-gray-900">Prompt refs</dt>
                  <dd>{adoptionCockpit.governancePanel.outputTraceability.promptRefs.length}</dd>
                </div>
              </dl>
              <p className="mt-3 text-xs text-gray-600">{adoptionCockpit.governancePanel.reviewGateControl.note}</p>
            </div>
            <div className="mt-4 space-y-3">
              <CommandList title="Power user groups" values={adoptionCockpit.powerUserGroups.map((group) => `${group.accountName}, ${group.practiceGroup}: ${group.runs} runs`)} />
              <CommandList title="Needs attention" values={adoptionCockpit.needsAttention} />
              <CommandList title="Recommended actions" values={adoptionCockpit.recommendedActions} />
              <CommandList title="Enabled releases" values={adoptionCockpit.releaseReadiness} />
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 overflow-x-auto rounded-lg border border-gray-100">
          ...
          <table className="w-full min-w-[860px] text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {adoptionCockpit.promptReadiness.map((item) => (
